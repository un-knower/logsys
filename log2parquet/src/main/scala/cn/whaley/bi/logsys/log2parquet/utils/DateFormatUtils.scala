...
object DateFormatUtils {
  ...
  val readFormatHour = new SimpleDateFormat("HH")
  ...
  def enHourAdd(dateStr: String, offset: Int) = {
    val date = readFormatHour.parse(dateStr)
    val cal = Calendar.getInstance()
    cal.setTime(date)
    cal.add(Calendar.HOUR, offset)
    readFormatHour.format(cal.getTime)
  }
