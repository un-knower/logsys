      ...
      foreachPartition(partition=>{
        ...
        partition.foreach(log=>{
          ...
          val info = fsMap.getOrElseUpdate(outpathPath,{
            val tmpFilePath = new Path(s"${outpathPath}/${partId}.json.gz")
            val jsonFilePath = new Path(s"${jsonDir}/key_appId2=${appId}/key_day=${key_day}/key_hour=${key_hour}/${appId}_${partId}.json.gz")
            fs.createNewFile(tmpFilePath)
            val stream = fs.append(tmpFilePath)
            val compressionOutputStream = new GzipCompressorOutputStream(stream)
